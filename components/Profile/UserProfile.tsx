...
const UserProfile: React.FC<{ userProfile: any }> = (props) => {
  ...
  // Convert userProfile to array if it's not already an array
  const profileArray = Array.isArray(props.userProfile)
    ? props.userProfile
    : props.userProfile
    ? [props.userProfile]
    : [];
      ...
      <Grid container>
        <Grid item xs={12} sm={6} md={4} order={{ xs: 2, md: 1 }}>
          ...
          {profileArray.length > 0 ? (
            <>
              <Typography className={classes.info}>User Name: {profileArray[0].userName}</Typography>
              <Typography className={classes.info}>First Name: {profileArray[0].firstName}</Typography>
              <Typography className={classes.info}>Last Name: {profileArray[0].lastName}</Typography>
              <Typography className={classes.info}>Email: {profileArray[0].email}</Typography>
              <br />
              <Typography className={classes.subTitleInfo}>Shipping Details</Typography>
              <Typography className={classes.info}>Delivery Address: {profileArray[0].deliveryAddress}</Typography>
              <Typography className={classes.info}>Postal Code: {profileArray[0].postalCode}</Typography>
              <Typography className={classes.info}>Contact Number: {profileArray[0].contactNumber}</Typography>
            </>
          ) : (
            <Typography className={classes.info}>No user data available</Typography>
          )}
          <br />
        </Grid>
        <Grid item xs={12} sm={6} md={4} order={{ xs: 3, md: 2 }}>
          <Typography className={classes.subTitleOrder}>Previous Orders</Typography>
          {paidCartArray.length > 0 ? (
              ...
              <div key={i} className={classes.eachPaidCart}>
                ...
                <div>
                  <Menu anchorEl={anchorEl && anchorEl[i]} open={Boolean(anchorEl && anchorEl[i])} onClose={handleClose}>
                    {paidCart.map((product: any, j: number) => (
                      <MenuItem key={j} className={classes.orderInfo}>
            ...
            <Typography className={classes.info}>No previous orders</Typography>
          )}
        </Grid>
        <Grid item xs={12} md={4} order={{ xs: 1, md: 3 }}>
          {profileArray.length > 0 && profileArray[0].userImage ? (
            <img src={profileArray[0].userImage} className={classes.userImage} alt="User Image" />
          ) : (
            <div className={classes.userImage}>No image</div>
          )}
        </Grid>
      </Grid>
    </Card>
