...
export const getServerSideProps: GetServerSideProps = async (context) => {
  ...
  if (!!Number(session?.id) === false) {
    const cartResult = await getOneCart(session?.id); // get cart from db to display during checkout.
    // Ensure serializable - handle undefined/null properly to prevent JSON errors
    const foundUser = cartResult?.body;
    const user = foundUser && typeof foundUser === 'object' ? JSON.parse(JSON.stringify(foundUser)) : null;
