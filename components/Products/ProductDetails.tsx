        ...
        <div className={classes.cardArea}>
          <Carousel className={classes.carousel}>
            {(() => {
              ...
              const productImages = Array.isArray(props.foundProduct.productImage) 
                ? props.foundProduct.productImage 
                : props.foundProduct.productImage 
                ? [props.foundProduct.productImage] 
                : [];
              
              // Normalize and use images
              const imagesToShow = productImages.length > 0 
                ? productImages 
                : ["/no-image.png"];
              
              return imagesToShow.map((imageUrl: string, i: number) => {
                // Normalize URL: encode spaces and add extension if needed
                let normalizedUrl = imageUrl;
                if (typeof normalizedUrl === 'string' && normalizedUrl.includes('res.cloudinary.com')) {
